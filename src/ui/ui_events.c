...
void on_validate_pin(lv_event_t *e)
{
    ...
    if (!validate_pin(pin))
    {
        numberOfWrongUnlockAttempts--;

        if (!numberOfWrongUnlockAttempts)
        {
        ...
        char message[128];
        sprintf(message, "Wrong PIN\nAttempts left: %d", numberOfWrongUnlockAttempts);
        lv_obj_center(lv_msgbox_create(NULL, "ERROR", message, NULL, true));
        return;
    }

    numberOfWrongUnlockAttempts = _maxNumberOfWrongUnlockAttempts;
